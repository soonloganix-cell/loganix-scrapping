const FatgridDomains = require('../models/FatgridDomains');

class FatgridDomainsRepository {
  constructor(sequelize) {
    this.fatgridDomainsModel = new FatgridDomains(sequelize);
    this.model = this.fatgridDomainsModel.getModel();
  }

  /**
   * Insert a single FatGrid domain record
   * @param {Object} domainData - Domain data object
   * @returns {Promise<Object>} Created domain record
   */
  async postFastGridDomain(domainData) {
    try {
      const domain = await this.model.create({
        id: domainData.id,
        url: domainData.url,
        bestPrice: domainData.bestPrice,
        class: domainData.class,
        currency: domainData.currency,
        resources: domainData.resources,
        dr: domainData.dr,
        linkFollow: domainData.linkFollow,
        authorityScore: domainData.authorityScore,
        organicTraffic: domainData.organicTraffic,
        backlinks: domainData.backlinks,
        refDomains: domainData.refDomains,
        db: domainData.db,
        categories: domainData.categories,
        totalTraffic: domainData.totalTraffic,
        traffic: domainData.traffic,
        totalOrganicTraffic: domainData.totalOrganicTraffic,
        createdAt: domainData.createdAt,
        bestNichePrices: domainData.bestNichePrices,
        tags: domainData.tags,
        note: domainData.note,
        isFavorite: domainData.isFavorite
      });
      ...
      console.error('Error creating FatGrid domain:', error);
      return {
        success: false,
        error: error.message,
  ...
  /**
   * Bulk create multiple FatGrid domain records
  ...
  async bulkCreate(domainsData) {
    try {
      ...
      const result = await this.model.bulkCreate(preparedData, {
        updateOnDuplicate: ['url', 'bestPrice', 'class', 'currency', 'resources', 'dr', 
                           'linkFollow', 'authorityScore', 'organicTraffic', 'backlinks', 
                           'refDomains', 'db', 'categories', 'totalTraffic', 'traffic', 
                           'totalOrganicTraffic', 'bestNichePrices', 'tags', 'note', 'isFavorite']
      });
      ...
      console.error('Error bulk creating FatGrid domains:', error);
      return {
        success: false,
        error: error.message,
  ...
  async sync(options = {}) {
    try {
      await this.fatgridDomainsModel.sync(options);
      return {
        success: true,
...
module.exports = FatgridDomainsRepository;
